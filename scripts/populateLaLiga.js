...
async function populateTeams() {
  ...
  const data = await fetchJSON(`${API_BASE}/competitions/PD/teams`);
...
async function populateMatches() {
  ...
  const data = await fetchJSON(`${API_BASE}/competitions/PD/matches`);
...
async function main() {
  try {
    ...
    console.error('Error populating La Liga data:', err.message);
  } finally {
    await pool.end();
  }
}
