...
async function populateTeams() {
  console.log('Fetching Serie A teams...');
  const data = await fetchJSON(`${API_BASE}/competitions/SA/teams`);
...
async function populateMatches() {
  console.log('Fetching Serie A matches...');
  const data = await fetchJSON(`${API_BASE}/competitions/SA/matches`);
...
async function main() {
  try {
    ...
    console.error('Error populating Serie A data:', err.message);
  } finally {
    await pool.end();
  }
}
