...
async function populateTeams() {
  ...
  const data = await fetchJSON(`${API_BASE}/competitions/PL/teams`);
...
async function populateMatches() {
  ...
  const data = await fetchJSON(`${API_BASE}/competitions/PL/matches`);
...
async function main() {
  try {
    ...
    console.error('Error populating EPL data:', err.message);
  } finally {
    await pool.end();
  }
}
