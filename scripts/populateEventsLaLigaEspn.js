...
// ESPN's undocumented public site API — no API key needed, no published rate limit,
// but be a good citizen: small delay between requests, and skip dates already processed
// so re-runs stay cheap. See project notes for the "resume where we left off" pattern
// used across the other populate* scripts.
...
// Pulls the season's full list of matchday dates from the league's "calendar" field,
// which ESPN includes on any scoreboard call (confirmed live on the eng.1 endpoint;
// esp.1 should carry the same field since it's the same site API pattern).
...
// Tracks which dates have already been fetched and processed, so re-runs only hit
// ESPN for genuinely new matchday dates. Created automatically if missing.
async function ensureTrackingTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS processed_event_dates (
...
async function getAlreadyProcessedDates() {
  const result = await pool.query(
    `SELECT date_str FROM processed_event_dates WHERE competition = $1`,
    [COMPETITION]
  );
...
async function markDateProcessed(dateStr) {
  await pool.query(
    `INSERT INTO processed_event_dates (date_str, competition) VALUES ($1, $2)
     ON CONFLICT (date_str, competition) DO NOTHING`,
    [dateStr, COMPETITION]
  );
}

// Maps ESPN's event flags/type to the event_type strings the rest of the app expects.
// Returns null for anything that isn't a goal or card (e.g. substitutions), so those
// are simply skipped rather than stored.
