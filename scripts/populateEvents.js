...
async function getFinishedMatches() {
  console.log('Fetching Champions League matches from Highlightly...');
  let offset = 0;
  const limit = 100;
  let all = [];
  ...
  console.log(`Found ${finished.length} finished League Phase matches out of ${all.length} total.`);
  return finished;
}

// Returns the set of highlightly_match_id values that already have at least one
// event row stored for UCL. A match's events are fetched in a single API call
  ...
  for (const ev of events) {
    ...
    await pool.query(
      `INSERT INTO match_events
        (highlightly_match_id, event_type, player_name, player_id, team_name, assisting_player_name, assisting_player_id, match_time)
...
async function main() {
  try {
    ...
    console.error('Error populating events:', err.message);
  } finally {
    await pool.end();
  }
}
